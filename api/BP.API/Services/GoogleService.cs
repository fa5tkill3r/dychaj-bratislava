...
public class GoogleService
{
    ...
    public async Task<Location?> GetLocation(double latitude, double longitude)
    {
        ...
        var locationResponse = await Requests.Get<GeocodeResponse>(
            $"https://maps.googleapis.com/maps/api/geocode/json?latlng={latitude},{longitude}&key={apiKey}");
        if (locationResponse == null || locationResponse.results.Count == 0)
            _logger.LogWarning("SensorCommunityService: Failed to get location for lat {Latitude} and long {Longitude}",
                latitude, longitude);
        else
            return new Location
            {
