...
public class ShmuWeatherService : IWeatherService
{
    ...
    {
        var shmuResponses = await Requests.Get<ShmuWeatherResponse>($"https://www.shmu.sk/api/v1/meteo/getweathergeojson");
        ...
        foreach (var sensor in sensors)
        {
            ...
            if (sensor.Type == ValueType.Pressure)
            {
                ...
                {
                    _logger.LogError("ShmuWeatherService: Sensor {SensorId} has invalid pressure data", sensor.UniqueId);
                }
    ...
    public async Task AddSensor(Module module, string uniqueId)
    {
        ...
        {
            var loc = await _googleService.GetLocation(feature.geometry.coordinates[1], feature.geometry.coordinates[0]);
